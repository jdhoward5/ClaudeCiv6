...
--
-- ClaudeAI MOD OVERRIDE: auto-confirms the handoff for Claude's controlled
-- seat only (so the AI can take its turn with no human click). Human seats are
-- unaffected and get the normal popup. The only change vs the base file is the
-- gated OnOk() call in ShowTurnControls(), keyed off the Game property
-- "ClaudeAI_AutoAdvanceSeat" set by the gameplay script in hot seat mode.
----------------------------------------------------------------
...
-- ===========================================================================
-- ClaudeAI: should the handoff for the current local player auto-advance?
-- True only in hot seat, only when the active seat is Claude's controlled seat
-- (published via the "ClaudeAI_AutoAdvanceSeat" Game property by the gameplay
-- script) and the seat has no hotseat password.
function ClaudeAI_ShouldAutoAdvance(localPlayerID, localPlayer)
	if (not GameConfiguration.IsHotseat()) then
		return false;
	end
	if (localPlayer == nil or localPlayer:GetHotseatPassword() ~= "") then
		return false;
	end
	local claudeSeat = nil;
	if (Game.GetProperty ~= nil) then
		claudeSeat = Game.GetProperty("ClaudeAI_AutoAdvanceSeat");
	end
	return claudeSeat ~= nil and tonumber(claudeSeat) == localPlayerID;
end
...
function ShowTurnControls()
	...
	if(not bPlayerChanging) then
		...
		-- ClaudeAI hot seat: auto-confirm the handoff for Claude's seat only.
		if (ClaudeAI_ShouldAutoAdvance(localPlayerID, localPlayer)) then
			print("[ClaudeAI] Auto-advancing hot seat handoff for Claude seat " .. tostring(localPlayerID));
			OnOk();
		end
	else
